...
3. **Free functions (data/formatting layer)** — pure-ish functions that call FastF1 (`load_event_names`, `load_results`, `load_best_lap_results`, `load_driver_details`, `load_track_map`, `build_qualifying_details`) and functions that format data into `rich.text.Text` for display (`format_position_cell`, `format_compounds`, `render_driver_details`, `render_track_panel`, `format_lap_time`, `format_delta`, etc.). These are kept separate from the App class specifically so they're unit-testable without spinning up the TUI — most of `tests/test_main.py` exercises this layer directly. Note the tests do not render widgets, so UI-visible regressions pass them; verify UI changes by driving the app with Textual's `run_test()` pilot.

The bottom strip is one `Horizontal` (`#bottom`) holding the track panel (`#track`) beside the log area. The track panel is the one panel that belongs to the *weekend* rather than to a session, so unlike the per-tab panels it lives on the app, is styled by unique id, and is fetched once per event — switching tabs must not redraw it, because the circuit cannot have changed.
...
The track panel breaks four of this project's own conventions, each for a measured reason. Read these before "fixing" any of them:
- **It reads FastF1's private `_api.position_data`, not `Session.get_telemetry()`.** For the 2026 season FastF1 3.7.0 raises `IndexError` parsing car data on *every* session, and because car data is loaded first that failure aborts the whole telemetry load and takes the position data down with it. The position data itself is fine when fetched directly. Restoring the public API disables the panel for the entire current season. The cost is real — a fastf1 upgrade can break this — and it is contained to `load_track_map`.
- **It does not use plotext.** plotext stretches data to fill its canvas, so the drawn shape is not the circuit's shape, and with three `scatter` calls only two colours survived. `project_track` is a hand-rolled grid instead. It scales both axes by one factor but converts the vertical span into character-cell units first (`CHARACTER_ASPECT`), because a cell is about twice as tall as it is wide — drop that and the circuit comes out squashed to half its height. An equal-aspect square circuit therefore fills only about 24 of its 36 columns; that whitespace is the price of an honest shape, not a bug.
- **Circuit lengths are hardcoded in `OFFICIAL_CIRCUIT_LENGTHS`, keyed by the F1 API circuit key.** Integrating the trace is systematically short, because a polyline cuts corners: measured at −1.94% at Monaco (65 m) and −1.74% at Montréal against +0.06% at the Hungaroring. A circuit missing from the table falls back to the traced figure, which is then labelled `≈`.
- **`pick_geometry_lap` takes the trace with the most distinct positions, not the fastest lap.** In a race the position feed updates any one car far less often than in qualifying: at the 2026 Hungarian Grand Prix the fastest lap carried 26 distinct positions where another car's carried 341, drawing the circuit as two dozen scattered dots. A circuit's shape does not depend on who drove it.
